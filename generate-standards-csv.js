...
execute('./scrape.sh', function() {
    ...
    substandards.forEach(function(standard) {
        ...
        if (standard.Description.match(/^\(\+\)/)) { // Check if it's a "transition" course: http://www.corestandards.org/Math/Content/note-on-courses-transitions/courses-transitions/
    ...
    // Write to a CSV file
    const fields = Object.keys(substandards[0]);
    const opts = { fields }
    try {
        const csv = parse(substandards, opts);
        fs.writeFileSync('standards.csv', csv);
    } catch (err) {
        console.error(err);
    }
});